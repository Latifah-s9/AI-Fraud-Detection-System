...
with st.sidebar:
    ...
    if hasattr(model, "predict_proba"):
        use_threshold = True
        thr = st.slider("Fraud threshold (probability)", 0.01, 0.99, 0.50, 0.01)
    else:
        thr = 0.50
...
if uploaded is not None:
    try:
        ...
        with c1: st.metric("Total Transactions", len(X_out))
        ...
        try:
            ...
            st.info(f"⚠️ SHAP not displayed: {str(e)}")

        # ✅ Performance metrics
        if "Class" in data_in.columns:
            try:
                y_true = data_in["Class"].values.astype(int)
                ...
                st.info(f"Metrics not computed: {str(e)}")
